...
void drawBlobsAndEdges(PImage source, BlobDetection bd0)
{
  //Pour chacun de blob dectecté par l'objet blobDetection
  for (int n=0; n<bd0.getBlobNb (); n++)
  {
    Blob b = bd0.getBlob(n); //Soit b le blob à l'index n du tableau de blob
    if (b!=null) //Si blob existe
    {
      drawBlob(false, b, source);
      drawCenter(false, b, source);
      drawEdges(true, b, source);
      drawBlobNumbre(false, b, n, source);
...
void drawSampleColor(boolean state, Blob b, PImage source)
{
  if (state)
  {
    ...
    sampleColorsBlob(b.xMin*source.width, b.xMax*source.width, b.yMin*source.height, b.yMax*source.height, 5, blobShape);
    blobShape.clear();
  }
}
...
boolean isBlobIsInsideAnotherBlob(Blob b1, Blob b0, PImage source)
{
  ArrayList<PVector> blob0Shape =  new ArrayList<PVector>();
  for (int m=0; m<b0.getEdgeNb (); m++)
  {
    EdgeVertex eA = b0.getEdgeVertexA(m); //Coordonées de départ du contour
    EdgeVertex eB = b0.getEdgeVertexB(m); //Coordonées d'arrivée du contour
    PVector va = new PVector(eA.x*source.width, eA.y*source.height);
    PVector vb = new PVector(eB.x*source.width, eB.y*source.height);
    blob0Shape.add(va);
    blob0Shape.add(vb);
  ...
  int nbVertInsideBlob0 = 0;
  for (int m=0; m<b1.getEdgeNb (); m++)
  {
    EdgeVertex eA = b1.getEdgeVertexA(m); //Coordonées de départ du contour
    EdgeVertex eB = b1.getEdgeVertexB(m); //Coordonées d'arrivée du contour
    PVector va = new PVector(eA.x*source.width, eA.y*source.height);
    PVector vb = new PVector(eB.x*source.width, eB.y*source.height);

    if (pixelInPoly(blob0Shape, va))
    {
      nbVertInsideBlob0++;
    }
  ...
  if (nbVertInsideBlob0 >= b0.getEdgeNb()/2)
  {
    return true;
  } else
  {
