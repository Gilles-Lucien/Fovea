...
void main()
{
	...
	float v = 1.0 / posProjection.w * ((posProjection.w - posProjection.y) * 0.5 + 0.5 * viewport.y * posProjection.w);
	vertTexCoord = texMatrix * vec4(u, v, 1.0, 1.0);

	dv = texture2D(displacementMap, vertTexCoord);
