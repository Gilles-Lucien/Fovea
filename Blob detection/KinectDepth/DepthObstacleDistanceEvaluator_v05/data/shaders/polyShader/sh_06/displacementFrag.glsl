...
void main()
{
	...
	if(dv.x > 0.0)
	{
		normal = normalize(ecNormal00);
	}
	else
	{
		normal = normalize(ecNormal01);
	}
	
	float intensity = max(0.0, dot(direction, normal)) * Kd * Ld;
	vec4 lightShadow = vec4(lightColor.x * intensity, lightColor.y * intensity, lightColor.z * intensity, 1.0);// * uv;
